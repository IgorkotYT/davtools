...
cleanup() {
    ...
    rm -f clean.txt clean.png tiny.png tiny.jpg invert.png img.gif test.pdf pdf.png test.mp4 mp4.gif base64.txt base64.txt.b64.txt base64.json base64.json.min.json server_test.log
}
trap cleanup EXIT
...
echo '{"a": 1, "b": " c "}' > minify.json
curl -s -T minify.json http://127.0.0.1:8081/convert/json-minify/in/minify.json
curl -s http://127.0.0.1:8081/convert/json-minify/out/minify.json.min.json --output minify.json.min.json
check_file minify.json.min.json

...
echo '{ "key" : "value" , "num" : 123 }' > test.json
...
# ensure the output is exactly correct without whitespace
if [ "$(cat test.min.json)" = '{"key":"value","num":123}' ]; then
    echo "SUCCESS: test.min.json content is minified properly"
else
    echo "FAILED: test.min.json content does not match expected output"
    exit 1
fi

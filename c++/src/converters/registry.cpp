...
std::vector<OutputArtifact> convert_base64_encode(const std::string&, const std::vector<std::uint8_t>&);
...
void init_registry_once_locked() {
    ...
    g_registry.emplace("base64-encode", Entry{convert_base64_encode, true, {}});
    ...
    try {
        if (op == "png-jpg") {
            ...
            if (out[0].data.find("YWJj\n") != 0) {
                throw std::runtime_error("base64-encode mismatch");
            }
        }
...
std::vector<std::string> canonical_ops_for_testing() {
    return {"png-jpg", "invert", "img-gif", "pdf-png", "mp4-gif", "virustest", "sha256", "base64-encode"};
}

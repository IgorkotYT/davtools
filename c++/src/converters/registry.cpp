...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            std::string json = "{\n  \"test\": \" a b c \"\n}";
            std::vector<uint8_t> in_bytes(json.begin(), json.end());
            auto out = g_registry.at(op).fn("selftest.json", in_bytes);
            if (out.empty() || out[0].data.empty()) throw std::runtime_error("empty output");
            if (out[0].data != "{\"test\":\" a b c \"}") {
                throw std::runtime_error("json minify mismatch");
            }

            auto out_empty = g_registry.at(op).fn("empty.json", {});
            if (out_empty.empty() || out_empty[0].data.empty() || out_empty[0].data != "{}") {
                throw std::runtime_error("json minify empty output mismatch");
            }
        }

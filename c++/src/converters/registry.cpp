...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            std::vector<std::uint8_t> in_data{' ', '{', '"', 'a', '"', ':', ' ', '1', '}', ' '};
            auto out = g_registry.at(op).fn("test.json", in_data);
            if (out.empty() || out[0].data != "{\"a\":1}") throw std::runtime_error("json-minify output mismatch");
        }

...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            auto out_empty = g_registry.at(op).fn("selftest.json", {});
            if (out_empty.empty()) throw std::runtime_error("empty output");
            if (out_empty[0].data != "") throw std::runtime_error("json-min empty input mismatch");
        }

...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            // test valid JSON
            std::string json_in = "{ \"key\" : \n \"value\" }";
            std::vector<std::uint8_t> json_vec(json_in.begin(), json_in.end());
            auto out = g_registry.at(op).fn("selftest.json", json_vec);
            if (out.empty() || out[0].data.empty()) throw std::runtime_error("empty output");
            if (out[0].data != "{\"key\":\"value\"}") {
                throw std::runtime_error("json minification mismatch");
            }
            // test empty input
            auto out_empty = g_registry.at(op).fn("empty.json", {});
            if (out_empty.empty()) throw std::runtime_error("empty array from empty input");
            if (!out_empty[0].data.empty()) throw std::runtime_error("non-empty string from empty input");
        }

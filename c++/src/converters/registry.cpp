...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            std::string test_json = "{ \"a\" : 1 }";
            std::vector<std::uint8_t> test_input(test_json.begin(), test_json.end());
            auto out = g_registry.at(op).fn("selftest.json", test_input);
            if (out.empty()) throw std::runtime_error("empty output");
            if (out[0].data != "{\"a\":1}") {
            ...
            if (out_empty.empty()) throw std::runtime_error("empty input failed to return artifact");
        }

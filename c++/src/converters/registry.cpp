...
std::vector<OutputArtifact> convert_minify_json(const std::string&, const std::vector<std::uint8_t>&);
...
void init_registry_once_locked() {
    ...
    g_registry.emplace("minify-json", Entry{convert_minify_json, true, {}});
...
void test_one(const std::string& op, bool disable_broken) {
    try {
        ...
        } else if (op == "minify-json") {
            auto out = g_registry.at(op).fn("selftest.json", {'{', '\n', ' ', '"', 'a', '"', ':', ' ', '1', '}'});
            if (out.empty() || out[0].data.empty()) throw std::runtime_error("empty output");
            if (out[0].data != "{\"a\":1}") {
                throw std::runtime_error("minify-json mismatch");
            }
        }
...
std::vector<std::string> canonical_ops_for_testing() {
    return {"png-jpg", "invert", "img-gif", "pdf-png", "mp4-gif", "virustest", "sha256", "base64", "minify-json"};
}

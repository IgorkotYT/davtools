...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            // Test valid JSON minification
            std::string input = "{\n  \"a\": 1\n}";
            std::vector<std::uint8_t> in_data(input.begin(), input.end());
            ...
            // Test empty input
            auto empty_out = g_registry.at(op).fn("selftest.json", {});
            if (empty_out.empty() || empty_out[0].data != "") {
                throw std::runtime_error("empty json input mismatch");
            }
        }

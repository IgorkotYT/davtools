...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            auto out = g_registry.at(op).fn("test.json", {'{', ' ', '}'});
            if (out.empty() || out[0].data.empty()) throw std::runtime_error("empty output");
...
std::vector<ConverterStatus> converter_self_test_all(bool disable_broken) {
    ...
    if (g_registry.count("json-min") && g_registry.count("json_min")) {
        g_registry["json_min"].enabled = g_registry["json-min"].enabled;
        g_registry["json_min"].reason  = g_registry["json-min"].reason;
    }
